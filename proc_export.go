...
func getStats(proc_map map[string]string) []byte {
  ...
  for k, path := range proc_map {
    ...
    if err == nil {
      ...
      response = setMetric(response, k + "_minflt", fields[9])
      response = setMetricType(response, k + "_cminflt", "gauge")
      response = setMetric(response, k + "_cminflt", fields[10])
      response = setMetricType(response, k + "_majflt", "gauge")
      response = setMetric(response, k + "_majflt", fields[11])
      response = setMetricType(response, k + "_cmajflt", "gauge")
      response = setMetric(response, k + "_cmajflt", fields[12])
      response = setMetricType(response, k + "_utime_seconds_total", "counter")
      response = setMetric(response, k + "_utime_secondsi_total", fields[13])
      response = setMetricType(response, k + "_stime_seconds_total", "counter")
      response = setMetric(response, k + "_stime_seconds_total", fields[14])
      response = setMetricType(response, k + "_nice", "gauge")
      response = setMetric(response, k + "_nice", fields[18])
      response = setMetricType(response, k + "_num_threads", "gauge")
      response = setMetric(response, k + "_num_threads", fields[19])
      response = setMetricType(response, k + "_vsize_bytes", "gauge")
      response = setMetric(response, k + "_vsize_bytes", fields[22])
      response = setMetricType(response, k + "_rss_pages", "gauge")
      response = setMetric(response, k + "_rss_pages", fields[23])
      response = setMetricType(response, k + "_rt_priority", "gauge")
      response = setMetric(response, k + "_rt_priority", fields[39])
